...
This repository (**06-dark-factory**) implements the **Sovereign Dark Factory**: an autonomous, local-first software factory orchestrator that coordinates local coding agents, executes deterministic verification gates, preserves patches, and provides human-in-the-loop review.
...
2. **Utilize our local inference skills** provided in `../05-local-coders`:
   - `local-coder`: Unified multi-engine router.
   - `ollama-coder`: Direct Ollama backend (`qwen2.5-coder:14b`, `llama3.1:8b`).
   - `foundry-coder`: Microsoft Foundry Local / Prism backend.
   - `prism`: Prism CUDA accelerator (`http://127.0.0.1:5272/v1`).
3. To generate code, run unit test authoring, or refactor components, invoke `ask_coder.py`:
   ```bash
   python3 ../05-local-coders/ask_coder.py code --task "<task description>" --output <file>
   ```
...
1. **Language & Runtime**: Python 3.12+ with strict type hinting (`from typing import ...`).
...
python3 ../05-local-coders/ask_coder.py status
